      ...
      >
        <template v-slot:additionalDescription>
          ...
          <ul>
            ...
            <li>
              {{ $t('参考値は、国の新型コロナウィルス感染症対策分科会で示された「今後想定される感染状況と対策について」のステージ1から4までのうち、ステージ3の指標を掲載。') }}
            </li>
            <li>
              {{ $t('陽性患者については、報道発表した数値を翌日の数値に反映している') }}
            </li>
            <li>
              {{ $t('（２）の数値は、｛直近1週間の累積新規陽性患者数/（本市人口/10万人）｝で計算') }}
            </li>
            <li>
              {{ $t('（３）の数値は、｛当該週の新規陽性患者数/前週の新規陽性患者数｝で計算') }}
            </li>
            <li>
              {{ $t('（４）の数値は、｛直近1週間の感染経路不明者/直近1週間の新規陽性患者数｝で計算') }}
            </li>
            <li>
              {{ $t('（５）の数値は、｛直近1週間の新規陽性患者数/市衛生研究所と民間検査機関の検査人数の合計｝で計算') }}
            </li>
            <li>
              {{ $t('（６）の数値は、｛当該週平日最終日時点の市公表分入院患者数｝である') }}
            </li>           
            <li>
